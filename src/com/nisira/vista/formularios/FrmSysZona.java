...
//    		listDAccionesVista = (new DaccionesvistaDao()).findAllVista(Inicio.idempresa, "FrmSysZona");
...
//    		listDAcciones = (new DACCIONESDao()).findDAcciones(Inicio.idempresa, "FrmSysZona");
			accionTransicion();
    	}catch(Exception ex){
    		ex.printStackTrace();
    	}
    ...
    public void cargarDatosMapa(){
    	try {
			...
			listGeneracionCodigo =(new GENERACIONCODIGOSDao()).listar(1,"IDEMPRESA=? AND PARAMETRO=? AND ESTADO=? AND IDPROCESO=?",ConfigInicial.LlenarConfig()[8],"FrmSysZona",1,programacionTarea.getIDPROCESO());
	        	...
	        	appListener = new NisiraLibGDX(observable_pos_actual,observable_Rutas){
			    	...
			    	public void LlenarZelda(){
			    		for(DZONAGENERAL obj:listDzonaGeneral){
			    			obj.getTIPORACKS();
			    			this.zelda.add(new Zelda(obj.getCORDENADAX(), obj.getCORDENADAY(),1, obj.getCOLOR(),obj.getTIPORACKS(),obj.getIDUBICACION()));
			    		}
			    	}
			    };
    	...
    	try {
    		...
    		if(Inicio.notificacion!=null){
    			...
    			if(programacionTarea!=null){
					...
					try {
						dzonageneral = dzonaGeneralDao.getDZonaGeneralxTarea(tarea.getIDEMPRESA(), tarea.getIDSUCURSAL(), tarea.getIDUBICACION());
        ...
        btnMcodbarra.addActionListener(new ActionListener() {
        	public void actionPerformed(ActionEvent e) {
						...
						if(this.listField!=null){
							if(this.listField.size()>0){
		    					...
		    					for(JTextLabelPanel obj:listField){
		    						...
		    						listTecladoMovil_CodigoBarra.add(new ObjetoTecladoMovil("CODIGO_BARRA",obj.descripcion,obj.id,obj.value));
				...
				for(GENERACIONCODIGOS gc : listGeneracionCodigo){
					manualCodigoBarra.listField.add(new JTextLabelPanel(pos++,gc.getIDGENERACION().toString(),gc.getDESCRIPCION(),"",gc.getNUMDIGITOTOTAL()){
						@Override
...
//					    }
//					}
					dacciones = daccionesDao.getPorClavePrimaria(Inicio.idempresa,accionSelection.getIDACCION(),actividades.getIDACTIVIDADES());
					Method  method =null;
					if(dacciones!=null){
						configActManulAutomatic = configActividadesDao.getConfigxActividades(actividades.getIDACTIVIDADES());
...
//						Method  method = FrmSysZona.class.getMethod(dacciones.getMETHODAUT());
//						method.invoke(FrmSysZona.this);
					}
				} catch ( SecurityException | SQLException | NisiraORMException | NoSuchMethodException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
				}
			}
        	...
        	layout.createParallelGroup(Alignment.TRAILING)
        		.addGroup(layout.createSequentialGroup()
        			...
        			.addGroup(layout.createParallelGroup(Alignment.TRAILING)
        				.addComponent(ScrollPaneZona, Alignment.LEADING)
        				.addGroup(layout.createSequentialGroup()
        					.addGroup(layout.createParallelGroup(Alignment.LEADING)
        						.addComponent(scrollPaneInformacion, GroupLayout.DEFAULT_SIZE, 470, Short.MAX_VALUE)
        					...
        					.addGroup(layout.createParallelGroup(Alignment.TRAILING)
        						...
        						.addComponent(panelHeaderMovilZona, GroupLayout.DEFAULT_SIZE, 424, Short.MAX_VALUE))))
        			.addContainerGap())
        );
        layout.setVerticalGroup(
        	layout.createParallelGroup(Alignment.LEADING)
        		.addGroup(layout.createSequentialGroup()
        			...
        			.addPreferredGap(ComponentPlacement.UNRELATED)
        			.addComponent(ScrollPaneZona, GroupLayout.DEFAULT_SIZE, 252, Short.MAX_VALUE)
        			.addContainerGap())
        );
...
//				objectPanelTexto[4] = "Ubicación : ";
				...
				/*OBTENER TOQUEN DE TAREA*/
					programacionTarea = (new ProgramaciontareaDao()).findProgramacionTarea(Inicio.idempresa, Inicio.idsucursal, 
							Inicio.notificacion.getTOQUEN(), Inicio.idmontacarga, 1);
			}else{
	...
	public DZONAGENERAL ProcesarRfid(List<TagsRfid> lst){
		...
		try {
			for(TagsRfid item : lst){/*SIZE -> NUMERO DE ANTENA 2 ANTENAS*/
				if(item.getTag()!=null){
					dasignacionchips = (new DASIGNACIONCHIPSDao()).getAsignacionChips(Inicio.idempresa, Inicio.idsucursal, item.getTag().getTagID());
...
//	        this.setFont(new Font("Tahoma", Font.PLAIN, 12));
	...
	class CargarDatosInicial implements Runnable{
		...
		public synchronized void run() {
			// TODO Auto-generated method stub
			/*DEFINIR THREAD*/
		    //progress= new ProgressBarDialog(Inicio.mainF,tareas,x,y,FrmSysZona.this);
...
//            			/*UBICACION*/
//            			mostrar+= "ANTENA ["+(rfid.getAntena().getIdantena()-1)+"] :"+rfid.getTag()+" ";
//            		}
    ...
    class CargarRutas implements Runnable{
		...
		public void run() {
			...
			try {
				...
				List<FLOYD> lstfloy=floydao.listar(1,"IDEMPRESA=? AND IDSUCURSAL=?",Inicio.idempresa,Inicio.idsucursal);
				
				if(lstfloy.size()==0){
					MatrizRutas.configuracionRutas(zonaGeneral, listDzonaGeneralCalles);
					System.out.println("FLOYD: S "+ MatrizRutas.f.S.length + " D "+MatrizRutas.f.D.length+
							" nodos "+ MatrizRutas.f.x);
					/*json*/
					...
					floydao.setPorClavePrimaria(floy.getIDEMPRESA(),floy.getIDSUCURSAL(),1,floy.getMATRIZ_S(),floy.getMATRIZ_D());
...
//    		panelZona.frmToltip.Closed();
    ...
    public String estructuraHtml(Object obj,int tipo){
		...
		case 1:
    		...
    		+ "<img src=\""+FrmSysZona.class.getResource("/resources/montacarga/montacargas3.png")+"\">"
			+ "</html>";
			break;
		case 2:
...
//		appListener.dispose();
	...
	public DiagramaDistribucion getPanelZona() {return null;
	}
...
//			observable_pos_actual.zelda.X; AQUI USAMOS LA POSICION ACTUAL PARA QUE SE ACTUALICE LA RUTA
//			Buscar el nodo del observable_posruta y luego el nodo del destino.
			System.out.println(fin.toString());
			//int inicio_pos = MatrizRutas.posicionMatrizRecorrido(observable_pos_actual.zelda.X,observable_pos_actual.zelda.Y);
			int fin_pos = 0;
			try{
