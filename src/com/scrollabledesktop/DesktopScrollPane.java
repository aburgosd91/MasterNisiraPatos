...
public class DesktopScrollPane extends JScrollPane {
	 ...
	 */
	public BaseInternalFrame add(DesktopListener dListener, String title,
			ImageIcon icon, JPanel frameContents, boolean isClosable, int x,
			int y) {

		BaseInternalFrame f = new BaseInternalFrame(title, icon, frameContents,
				isClosable);
	...
	public void flagContentsChanged(JInternalFrame f) {
		if (desktopPane.getSelectedFrame() != f) {
			BaseToggleButton button = (BaseToggleButton) ((BaseInternalFrame) f)
					.getAssociatedButton();
