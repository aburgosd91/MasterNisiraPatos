...
public abstract class AbstractMaestro extends NSRInternalFrame {
	...
	public void DoEliminar() {
		if (Inicio.usuario.getGrupousuario_fk_usuario_grupousuario().getEs_administrador() == 0) {
				//&& privilegio.getEliminar() == 0) {
			UtilMensajes.mensaje_alterta("NOPRIV_ELIMINAR");
			return;
		}
		if (UtilMensajes.mensaje_sino("DESEA_ELIMINAR_REG") == 0) {
			eliminar();
			setEstado(VISTA);
			getBarra().enVista();
			vista_noedicion();
	...
	KeyStroke editarKey = KeyStroke.getKeyStroke(KeyEvent.VK_E, InputEvent.CTRL_MASK);
	KeyStroke nuevoKey = KeyStroke.getKeyStroke(KeyEvent.VK_N, InputEvent.CTRL_MASK);
	KeyStroke grabarKey = KeyStroke.getKeyStroke(KeyEvent.VK_G, InputEvent.CTRL_MASK);
	KeyStroke cancelarKey = KeyStroke.getKeyStroke(KeyEvent.VK_U, InputEvent.CTRL_MASK);
	KeyStroke elminarKey = KeyStroke.getKeyStroke(KeyEvent.VK_B, InputEvent.CTRL_MASK);
}